...
void loop( )
{
	...
	while( 1 )
	{
		if ( loopCounter == 0 )
		{
			if ( entropyBufferFill( ) )
			{
				writeLed( 0xFF );
				writeLed( 0x00 );
			}
			//Serial.println( entropyBufferAvailable( ) );
		}
		loopCounter++; // Intentional overflow!
		...
		if ( !adminMode )
		{
			...
			{
				Serial.println( "Invalid CRC, tamper detected!" );
				tampered = 1;
				delay( 5000 );
			}
			else if ( readKeyResult == READ_KEY_SUCCESS )
			{
				if ( ! findUid( id, &uid ) )
				{
					Serial.println( "Invalid UID, tamper detected!" );
					tampered = 1;
					delay( 5000 );
				}
				else 
